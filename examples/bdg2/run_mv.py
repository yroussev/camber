...
CHILLED = [
    ...
    "Fox_lodging_Stephan",
    "Eagle_health_Athena",
    "Bull_education_Magaret",
    "Hog_education_Hallie",
]
...
def mv_table(meta, weather, meter_csv, buildings, label):
    ...
    present = [b for b in buildings if b in pd.read_csv(meter_csv, nrows=0).columns]
    df = pd.read_csv(
        meter_csv, usecols=["timestamp"] + present, parse_dates=["timestamp"]
    ).set_index("timestamp")
    print(f"\n=== M&V: {label} (daily change-point, CV(RMSE) accept <= {cvmax:.0%}) ===")
    print(f"{'building':26s} {'site':9s} {'model':5s} {'n':>4s} {'R2':>5s} {'CVRMSE':>7s} acc")
    for b in present:
        e = df[b].loc[YEAR[0] : YEAR[1]].dropna()
        if len(e) < 24 * 150:
            continue
        d = daily_energy_vs_temp(e, _oat_f(weather, _site_of(meta, b)), rate_is_energy_rate=False)
        if len(d) < 60:
            continue
        ...
        print(
            f"{b:26s} {_site_of(meta, b):9s} {m.kind:5s} {st.n:>4d} "
...
def towt_demo(meta, weather, elec_csv):
    ...
    candidates = [c for c in preferred if c in hdr] or [c for c in hdr if "_office_" in c][:6]
    df = pd.read_csv(
        elec_csv, usecols=["timestamp"] + candidates, parse_dates=["timestamp"]
    ).set_index("timestamp")
...
def storage_demo(meta, elec_csv, buildings):
    ...
    present = [b for b in buildings if b in pd.read_csv(elec_csv, nrows=0).columns]
    df = pd.read_csv(
        elec_csv, usecols=["timestamp"] + present, parse_dates=["timestamp"]
    ).set_index("timestamp")

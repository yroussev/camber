...
public labeled faults; this one deterministically scores *every covered rule* against its own injected
fault (and a clean frame), so the whole single-equipment suite is accuracy-measured with no download.
Reuses ``camber.eval.benchmark`` (the LBNL evaluation method) and gates against a committed baseline.
    ...
    python examples/synthetic_fdd/benchmark.py --update-baseline examples/synthetic_fdd/benchmark-baseline.json
"""
...
def metrics_dict() -> dict:
    ...
    o = rep.overall
    m = {"overall.tpr": round(o.true_positive_rate, 4),
         ...
         "g36.tpr": g36["tpr"], "g36.fpr": g36["fpr"], "g36.n_fc": g36["n_fc"]}
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--gate", metavar="PATH", help="baseline JSON to gate against (exit 2 on regression)")
    ap.add_argument("--tol", type=float, default=0.0, help="regression tolerance (default 0.0 — deterministic)")
    ap.add_argument("--update-baseline", metavar="PATH", help="write current metrics as a new baseline")
    args = ap.parse_args(argv)

    rep = benchmark(faultlab.labeled_records(), faultlab.targets())
    cov = faultlab.coverage()
    o = rep.overall
    print(f"Synthetic FDD accuracy — {len(cov['scored'])}/{cov['n_single']} single-equipment rules scored")
    print(f"  overall: TPR {o.true_positive_rate:.0%}  FPR {o.false_positive_rate:.0%}  "
    ...
    for fc, d in sorted(g36["per_fc"].items()):
        print(f"  FC{fc:<2} faulty {d['faulty_pct']:5.0f}%  clean {d['clean_pct']:4.0f}%  "
              f"{'detected' if d['detected'] else 'MISSED':8s} {'quiet' if d['clean_quiet'] else 'NOISY'}")
    ...
    if xf:
        print("\nCo-detections (a scenario legitimately containing a second condition — not a false positive):")
    ...
    if args.gate:
        ...
        if not chk.passed:
            ...
            return 2
        print(f"\n✓ gate OK — {chk.unchanged} stable, {len(chk.improvements)} improved, none regressed")
    return 0

...
- [x] **Sensor health & data-trust layer** — sensor faults are not equipment faults;
      ...
      external weather such as NASA POWER, a station, or a TMY series; the weather fetch
      itself now ships as `camber.weather_source`, dependency-light with an injectable
      transport, spanning **two keyless providers** — NASA POWER (global reanalysis grid)
      and NOAA/ISD (real weather stations, via `oat_reference_isd`) — see
      [docs/WEATHER.md](docs/WEATHER.md)); and

...
| `chiller_efficiency`, `cooling_tower_approach` | **measured on the LBNL chiller-plant subset** (opt-in `fetch.py --chiller`, not gated in CI): `chiller_efficiency` **TPR 5/9, FPR 1/2**; `cooling_tower_approach` **TPR 0/4, FPR 4/7** | kW/ton catches every three-way-bypass fault and misses tower fouling / PI mistuning (≤ 7% kW/ton change); its false alarm is the chiller sensor-bias run, whose biased temperature corrupts the tonnage. The tower rule never ran before 0.82.0 (a 0–1 tower fan speed was never rescaled, and the dataset's wet-bulb and dry-bulb labels are swapped — see `mapping_chiller.json`). Now it runs: fouling and PI mistuning move the approach by only ~1 °F, under its 3 °F margin, and it fires on 4 of the 5 bypass runs, where the whole plant restages — why the approach rises there is not established, so those stay negatives |

...
demonstrated. These numbers are gated in CI by `examples/lbnl_fdd/benchmark-baseline.json`.

| Detector | On LBNL SDAHU | Why |
|---|---|---|
| `coil_valve_drift` | **measured: recall 0/1**, 0 false positives in 3 scored negatives; 2 cross-negatives declined (stuck-open damper → nothing scoreable) | all required points mapped, but the labeled 50% coil leak moves the valve only **+1% (0.2σ)** against the frozen baseline at matched air-ΔT — well inside the healthy band, so it is not detected |
| `economizer_damper_drift` | **measured: recall 0/4**, 0 false positives in 2 negatives | the drift moves the right way on the larger faults (stuck at 25%: **−12 pp, −2.1σ**; stuck at 100%: **+11 pp, +1.9σ**) but stays under the warn threshold; also fires cleanly only if `OA_DMPR` is the *command* — if it's the stuck actual position, it reduces to the level check `outdoor_air_fraction` already covers |
...
| `vav_airflow_drift`, `vav_reheat_valve_drift` | **measured on the LBNL FPU subset: recall 0/4 and 0/5**, 0 false positives (opt-in `fetch.py --fpu`, not gated in CI) | the fan-power-unit subset carries per-box damper / airflow+setpoint / reheat-valve / discharge-temp. Airflow drift barely moves on its targets (damper stuck at 100%: **0.9σ**); reheat-valve drift reads **−0%** on every reheat fault, *including a valve stuck fully open*, which points at the detector or the mapping rather than at sensitivity -- under investigation |
| `chiller_efficiency`, `cooling_tower_approach` | **measured on the LBNL chiller-plant subset** (opt-in `fetch.py --chiller`, not gated in CI): `chiller_efficiency` **TPR 5/9, FPR 1/2**; `cooling_tower_approach` **does not score** — the benchmark skips it ("no healthy baseline metric") | the plant subset carries chiller power + CHW loop and tower supply-temp + wet-bulb; each detector's absolute design ceiling is calibrated from the fault-free run (commissioning-style), then scored against the labeled physical faults — tower fouling / PID for the tower approach, plus three-way-bypass leak/stuck for chiller kW/ton |
...
| `dcv_verification`, `dcv_system_verification` | **synthetic-only (physics-simulated)** | no licence-clean public dataset carries both CO₂ and OA trends — the LBNL SDAHU/DDAHU/FCU exports have OA flow and damper position but **no CO₂ column**; scored on `faultlab.dcv_sim`, a zone CO₂ mass balance under proportional / integral / static DCV with an economizer, warm-up closures and a fan schedule |

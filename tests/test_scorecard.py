...
from camber.scorecard import (  # noqa: E402
    CATEGORIES, Scorecard, build_scorecard, category_for, grade_for,
)
...
def test_penalties_by_category_and_severity():
    sc = build_scorecard([_f("simultaneous_heat_cool", "fault"),   # energy -15
                          _f("unmet_setpoint_hours", "warn")])     # comfort -5
...
def test_weights_and_jsonable():
    ...
    heavy = build_scorecard(findings, category_weights={"energy": 10, "comfort": 1,
                                                        "ventilation": 1, "maintenance": 1})
    light = build_scorecard(findings)
    assert heavy.overall_score < light.overall_score              # weighting energy drags it down
    assert "categories" in heavy.as_dict() and heavy.as_dict()["overall_grade"] == heavy.overall_grade

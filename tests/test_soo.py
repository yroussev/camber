...
def test_predicate_ops():
    ...
    _, w = Predicate(Role.SUPPLY_AIR_TEMP, "within", ref=Role.SUPPLY_AIR_TEMP_SP, tol=2.0).evaluate(
        f
    )
...
def test_persistence_forgives_transient_violations():
    ...
    f = pd.DataFrame(
        {Role.SUPPLY_AIR_TEMP: sat, Role.SUPPLY_AIR_TEMP_SP: np.full(n, 55.0)}, index=_idx(n)
    )
    expect = Predicate(Role.SUPPLY_AIR_TEMP, "within", ref=Role.SUPPLY_AIR_TEMP_SP, tol=2.0)
...
def test_soo_findings_shape():
    n = 100
    f = pd.DataFrame(
        {Role.OAT: np.array([70.0] * 60 + [50.0] * 40), Role.BOILER_STATUS: np.zeros(n)},
        index=_idx(n),
    )
    spec = [
        Clause(
    ...
    c = clause_from_dict(
        {
            "name": "y",
            "expect": {
                "subject": "supply_air_temp",
                "op": "within",
                "ref": "supply_air_temp_sp",
                "tol": 1.5,
            },
        }
    )

...
def test_heatpump_defrost_fault_and_clean():
    ...
    clean = pd.DataFrame(
        {
            Role.REVERSING_VALVE_CMD: pd.Series(
                np.concatenate([np.ones(n // 2), np.zeros(n - n // 2)]), index=_FINE
            )
        }
    )
...
def test_chiller_approach_fault_and_clean():
    fault = pd.DataFrame(
        {
            Role.COND_APPROACH_TEMP: pd.Series(12.0, index=_HR),
            Role.EVAP_APPROACH_TEMP: pd.Series(9.0, index=_HR),
        }
    )
    clean = pd.DataFrame(
        {
            Role.COND_APPROACH_TEMP: pd.Series(4.0, index=_HR),
...
def test_chiller_approach_info_without_inputs():
    f = ChillerApproachFouling().analyze(
        "CH1", pd.DataFrame({Role.POWER: pd.Series(100.0, index=_HR)})
    )
    assert f.severity == "info"

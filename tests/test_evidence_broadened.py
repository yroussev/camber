    ...
    return pd.DataFrame(
        {
            ...
            Role.OAT: oat,
            Role.SUPPLY_FAN_SPEED: pd.Series(
                np.where((idx.hour >= 6) & (idx.hour <= 20), 0.8, 0.2), index=idx
            ),
...
def test_declined_hook_falls_back_to_default_or_none():
    ...
    assert (
        finding_evidence(ReheatPenalty(), "AHU-1", frame[[Role.HEAT_VALVE]]).renderer
        == "multitrend"
    )
...
def test_audit_embeds_evidence_with_rules_and_frames():
    ...
    rep.add_findings(
        [
            Finding(
                rule="reheat_penalty", equip="AHU-1", severity="fault", summary="reheat at high OAT"
            ),
            Finding(
                rule="night_weekend_setback", equip="AHU-1", severity="warn", summary="no setback"
            ),
        ]
    )
    plain = rep.to_html()
...
def test_audit_evidence_skips_finding_without_a_frame():
    ...
    rep.add_findings(
        [
            ...
            Finding(
                rule="reheat_penalty", equip="AHU-2", severity="fault", summary="y"
            ),  # no frame
        ]
    )

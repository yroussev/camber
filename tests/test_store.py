...
def test_hive_partitions_on_disk(tmp_path):
    ...
    st.write_role_frame(
        _frame(start="2024-12-30", periods=96), site="DemoSite", equip="AHU_1", equip_class="AHU"
    )  # spans 2024 -> 2025
...
def test_read_filters_by_role_and_time(tmp_path):
    ...
    long = st.read_long(
        site="S", roles=[Role.HEAT_VALVE], start="2024-01-01 06:00", end="2024-01-01 09:00"
    )
...
def test_append_accumulates_without_clobber(tmp_path):
    st = ParquetStore(str(tmp_path / "tsdb"))
    st.write_role_frame(
        _frame(start="2024-01-01", periods=5), site="S", equip="AHU_1", equip_class="AHU"
    )
    st.write_role_frame(
        _frame(start="2024-02-01", periods=5), site="S", equip="AHU_1", equip_class="AHU"
    )
...
def test_resample_on_read(tmp_path):
    st = ParquetStore(str(tmp_path / "tsdb"))
    st.write_role_frame(_frame(periods=48, freq="1h"), site="S", equip="AHU_1", equip_class="AHU")
...
def test_prune_removes_old_year_partitions(tmp_path):
    ...
    # spans 2024 -> 2025
    st.write_role_frame(
        _frame(start="2024-12-30", periods=96), site="S", equip="AHU_1", equip_class="AHU"
    )

...
def test_role_frame_to_long_empty():
    long = role_frame_to_long(pd.DataFrame(), equip="E")
    assert long.empty
...
def test_write_then_read_role_frame_roundtrips(tmp_path):
    ...
    f = _frame(periods=24)
    n = st.write_role_frame(f, facility_id="DemoSite", equip="AHU_1", equip_class="AHU")
    assert n == 48  # 24 rows * 2 roles
    back = st.read_role_frame(facility_id="DemoSite", equip="AHU_1")
    assert list(back.columns) == [Role.COOL_VALVE, Role.HEAT_VALVE] or set(back.columns) == {
        Role.HEAT_VALVE,
        Role.COOL_VALVE,
    }
    assert len(back) == 24
...
def test_hive_partitions_on_disk(tmp_path):
    ...
    st.write_role_frame(
        _frame(start="2024-12-30", periods=96),
        facility_id="DemoSite",
        equip="AHU_1",
        equip_class="AHU",
    )  # spans 2024 -> 2025
    sites = [d for d in os.listdir(root) if d.startswith("facility_id=")]
    assert sites == ["facility_id=DemoSite"]
    years = sorted(os.listdir(os.path.join(root, "facility_id=DemoSite")))
...
def test_read_filters_by_role_and_time(tmp_path):
    st = ParquetStore(str(tmp_path / "tsdb"))
    st.write_role_frame(_frame(periods=48), facility_id="S", equip="AHU_1", equip_class="AHU")
    long = st.read_long(
        facility_id="S", roles=[Role.HEAT_VALVE], start="2024-01-01 06:00", end="2024-01-01 09:00"
    )
...
def test_read_filters_by_equip(tmp_path):
    st = ParquetStore(str(tmp_path / "tsdb"))
    st.write_role_frame(_frame(periods=10), facility_id="S", equip="AHU_1", equip_class="AHU")
    st.write_role_frame(_frame(periods=10), facility_id="S", equip="AHU_2", equip_class="AHU")
    long = st.read_long(facility_id="S", equips=["AHU_2"])
...
def test_append_accumulates_without_clobber(tmp_path):
    st = ParquetStore(str(tmp_path / "tsdb"))
    st.write_role_frame(
        _frame(start="2024-01-01", periods=5), facility_id="S", equip="AHU_1", equip_class="AHU"
    )
    st.write_role_frame(
        _frame(start="2024-02-01", periods=5), facility_id="S", equip="AHU_1", equip_class="AHU"
    )
    long = st.read_long(facility_id="S", roles=[Role.HEAT_VALVE])
...
def test_resample_on_read(tmp_path):
    st = ParquetStore(str(tmp_path / "tsdb"))
    st.write_role_frame(
        _frame(periods=48, freq="1h"), facility_id="S", equip="AHU_1", equip_class="AHU"
    )
    daily = st.read_role_frame(facility_id="S", equip="AHU_1", resample="1D")
...
def test_points_and_sites_catalog(tmp_path):
    st = ParquetStore(str(tmp_path / "tsdb"))
    st.write_role_frame(_frame(periods=4), facility_id="A", equip="AHU_1", equip_class="AHU")
    st.write_role_frame(_frame(periods=4), facility_id="B", equip="AHU_9", equip_class="AHU")
    assert sorted(st.facilities()) == ["A", "B"]
    keys = st.points(facility_id="A")
    assert {(k.equip, k.role) for k in keys} == {("AHU_1", "heat_valve"), ("AHU_1", "cool_valve")}


def test_read_empty_store_is_empty(tmp_path):
    st = ParquetStore(str(tmp_path / "nope"))
    assert st.read_long(facility_id="S").empty
    assert st.read_role_frame(facility_id="S", equip="X").empty
    assert st.facilities() == []
    assert st.points() == []
...
def test_write_rollup_to_dest_store(tmp_path):
    ...
    src.write_role_frame(_frame(periods=48), facility_id="S", equip="AHU_1", equip_class="AHU")
    dest = ParquetStore(str(tmp_path / "dest"))
    n = src.write_rollup("D", dest, agg="mean")
    assert n == 4
    back = dest.read_role_frame(facility_id="S", equip="AHU_1")
...
def test_prune_removes_old_year_partitions(tmp_path):
    ...
    # spans 2024 -> 2025
    st.write_role_frame(
        _frame(start="2024-12-30", periods=96), facility_id="S", equip="AHU_1", equip_class="AHU"
    )

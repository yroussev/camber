...
def test_insufficient_data_returns_none():
    n = 24
    assert (
        analyze_cw_reset(pd.DataFrame({"CWS_Temp": np.full(n, 85.0)}, index=_idx(n)), "CWP") is None
    )
...
def test_rule_is_a_rule_and_severity():
    ...
    reset = pd.DataFrame(
        {
            Role.CW_SUPPLY_TEMP: wb + 7.0,
            Role.CW_RETURN_TEMP: wb + 17.0,
    ...
    flat = pd.DataFrame(
        {
            Role.CW_SUPPLY_TEMP: np.full(n, 85.0),

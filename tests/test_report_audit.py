...
from camber.report.audit import ECM, AuditReport, Benchmark  # noqa: E402
...
def test_text_report_contains_sections():
    r = AuditReport(
        building="DemoBuilding", level=2, climate_zone="CA CZ15", benchmark=Benchmark(120.0, 100.0)
    )
    r.add_ecm(
        ECM(
            "SAT reset",
...
def test_html_report_well_formed_and_escaped():
    r = AuditReport(building="A & B <DemoBuilding>", level=3, benchmark=Benchmark(120.0, 100.0))

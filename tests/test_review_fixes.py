...
def test_interval_hours_survives_duplicate_timestamps():
    ...
    idx = pd.DatetimeIndex(["2025-11-02 01:00", "2025-11-02 01:00", "2025-11-02 02:00",
                            "2025-11-02 03:00"])
...
def test_cohort_flags_outlier_when_majority_share_a_value():
    ...
    frames = {f"VAV-{i}": pd.DataFrame({Role.AIRFLOW: pd.Series(1000.0, index=idx)})
              for i in range(8)}
...
def test_savings_chart_no_crash_on_all_nan_report():
    ...
    ax, res = savings_chart(model, rng.uniform(20, 90, 30), yr, n_baseline=100, p_baseline=2,
                            cv_rmse=0.05)                    # was IndexError on cum_avoided[-1]
    assert ax is not None


def test_scorecard_counts_unmapped_rule():
    from camber.scorecard import build_scorecard
    sc = build_scorecard([Finding(rule="some_plugin_rule", equip="E", severity="fault", summary="")])

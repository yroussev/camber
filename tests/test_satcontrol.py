...
def _frame(sat, sp=55.0, fan=1.0, n=240):
    ...
    return pd.DataFrame({Role.SUPPLY_AIR_TEMP: sat, Role.SUPPLY_AIR_TEMP_SP: pd.Series(sp, index=idx),
...
def test_only_running_hours_counted():
    ...
    frame = pd.DataFrame({Role.SUPPLY_AIR_TEMP: sat, Role.SUPPLY_AIR_TEMP_SP: pd.Series(55.0, index=idx),

...
def test_render_evidence_multitrend_with_mask():
    ...
    ev = Evidence(
        renderer="multitrend",
        roles=[Role.HEAT_VALVE, Role.COOL_VALVE],
        mask=mask,
        label="both_open",
    )
...
def test_evidence_descriptor_is_jsonable():
    ...
    assert len(d["violations"]) == int(mask.sum()) and all(
        isinstance(t, str) for t in d["violations"]
    )
...
def test_default_evidence_covers_rules_without_a_hook():
    ...
    frame = pd.DataFrame(
        {
            Role.POWER: pd.Series(100.0, index=idx),
...
def test_default_evidence_none_when_no_required_role_present():
    ...
    assert (
        finding_evidence(
            ChillerEfficiency(), "CH-1", pd.DataFrame({Role.OAT: pd.Series(70.0, index=idx)})
        )
        is None
    )

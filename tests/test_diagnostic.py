...
from camber.charts.diagnostic import (  # noqa: E402
    TEMPLATES, DiagnosticTemplate, band, diagnostic_scatter, reset_line,
)
...
def test_sat_reset_conforming_vs_flat():
    oat, rng, idx = _oat()
    good = pd.Series(65 + (55 - 65) / 60 * oat.clip(0, 60) + rng.normal(0, 0.5, len(oat)), index=idx)
    flat = pd.Series(np.full(len(oat), 55.0) + rng.normal(0, 0.5, len(oat)), index=idx)
    _, m_good = diagnostic_scatter(pd.DataFrame({Role.OAT: oat, Role.SUPPLY_AIR_TEMP: good}),
                                   TEMPLATES["sat_reset"])
    _, m_flat = diagnostic_scatter(pd.DataFrame({Role.OAT: oat, Role.SUPPLY_AIR_TEMP: flat}),
                                   TEMPLATES["sat_reset"])
...
def test_economizer_stuck_minimum_flagged():
    ...
    _, m_good = diagnostic_scatter(pd.DataFrame({Role.OAT: oat, Role.OA_DAMPER: good}),
                                   TEMPLATES["economizer"])
    _, m_stuck = diagnostic_scatter(pd.DataFrame({Role.OAT: oat, Role.OA_DAMPER: stuck}),
                                    TEMPLATES["economizer"])
...
def test_no_simultaneous_heat_cool_flagged():
    ...
    _, m_ok = diagnostic_scatter(pd.DataFrame({Role.COOL_VALVE: cool, Role.HEAT_VALVE: heat_ok}),
                                 TEMPLATES["no_simultaneous_hc"])
    _, m_bad = diagnostic_scatter(pd.DataFrame({Role.COOL_VALVE: cool, Role.HEAT_VALVE: heat_bad}),
                                  TEMPLATES["no_simultaneous_hc"])
...
def test_returns_axes_and_mask_aligned_to_index():
    ...
    ax, mask = diagnostic_scatter(pd.DataFrame({Role.OAT: oat, Role.SUPPLY_AIR_TEMP: y}),
                                  TEMPLATES["sat_reset"])

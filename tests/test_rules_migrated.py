...
from camber.rules.base import FleetRule, Registry, Rule  # noqa: E402
...
def test_reheat_ok_when_no_reheat():
    n = 24 * 21
    frame = pd.DataFrame({Role.HEAT_VALVE: np.zeros(n), Role.OAT: np.full(n, 95.0)}, index=_idx(n))
    ...
    a = pd.DataFrame(
        {
            ...
            Role.AIRFLOW_SP: np.full(n, 200.0),
        },
        index=idx,
    )  # A both
    b = pd.DataFrame(
...
def test_registry_holds_all_migrated():
    ...
    for r in (ReheatPenalty(), SupplyAirReset(), ZonesHeatCoolCensus()):
        reg.register(r)
    assert set(reg.names()) == {"reheat_penalty", "supply_air_reset", "zones_heat_cool_census"}

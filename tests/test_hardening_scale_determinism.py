...
def test_fleet_report_scales_to_500_buildings():
    ...
    buildings = [{"site": f"B{i}", "eui": float(e),
                  "findings": [Finding(rule="simultaneous_heat_cool", equip="AHU",
                                       severity="fault", summary="x")]}
...
def test_fleet_percentile_matches_bruteforce():
    ...
    bs = [{"site": f"B{i}", "eui": float(e), "findings": []}
...
def _cal():
    from camber.mandv.rc_model import calibrate, daily_schedule, RCModel
...
def _cohort():
    ...
    frames = {f"V{i}": pd.DataFrame({Role.AIRFLOW: np.full(48, 100.0 + 40 * (i == 3))}, index=idx)
              for i in range(6)}
...
@pytest.mark.parametrize("fn", [_cal, _best_model, _level_shifts, _cohort, _faultlab],

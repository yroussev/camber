...
def _occ_frame(n, space, cool=74.0, heat=None, freq="1h"):
    ...
    data = {"SpaceTemp": np.full(n, space, dtype=float), "ActCoolSP": np.full(n, cool, dtype=float)}
    if heat is not None:
...
def test_three_tiers_absolute():
    ...
    r = analyze_overcooling_severity(_occ_frame(n, 71.0), "VAV_fault", occupied_only=False)
    ...
    r = analyze_overcooling_severity(_occ_frame(n, 72.0), "VAV_warn", occupied_only=False)
    assert r.severity == "warn"
    assert not r.tier_sustained["fault"] and r.tier_sustained["warn"]

    # 73 vs 74 = 1 degF -> info only
    r = analyze_overcooling_severity(_occ_frame(n, 73.0), "VAV_info", occupied_only=False)
    assert r.severity == "info"
    assert not r.tier_sustained["warn"] and r.tier_sustained["info"]

    # at setpoint -> nothing
    r = analyze_overcooling_severity(_occ_frame(n, 74.0), "VAV_ok", occupied_only=False)
...
def test_persistence_gap_does_not_fake_window():
    ...
    t = pd.to_datetime(
        [
            "2025-07-07 10:00",
            "2025-07-07 10:15",  # 2 samples, then gap
            "2025-07-07 14:00",
            "2025-07-07 14:15",  # 2 samples
        ]
    )
    df = pd.DataFrame(
        {"SpaceTemp": [71.0, 71.0, 71.0, 71.0], "ActCoolSP": [74.0, 74.0, 74.0, 74.0]}, index=t
    )
    r = analyze_overcooling_severity(
        df, "VAV", window_min=60.0, interval="15min", occupied_only=False
    )
...
def test_relative_mode_uses_heating_setpoint():
    ...
    rel = analyze_overcooling_severity(frame, "VAV", relative_to_deadband=True, occupied_only=False)
    assert rel.mode == "relative_deadband"
    assert rel.severity == "ok"  # inside the deadband

    ab = analyze_overcooling_severity(frame, "VAV", relative_to_deadband=False, occupied_only=False)
...
def test_falls_back_to_absolute_without_heating_setpoint():
    ...
    r = analyze_overcooling_severity(frame, "VAV", relative_to_deadband=True, occupied_only=False)

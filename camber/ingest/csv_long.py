...
Many historians and BAS "export all points" tools emit a **narrow** table (``timestamp,point,value``,
optionally ``unit``) rather than a wide one. This pivots it to the standard `SourceAdapter` wide,
datetime-indexed, numeric frame, reusing the shared timestamp/value parsers and honoring a vendor
:mod:`~camber.ingest.profiles` profile. (The SQL adapter already does this from a DB connection; this is
the CSV-file equivalent.)
"""
...
class LongCsvAdapter:
    ...
    def __init__(self, path: str, *, ts_col: str = "timestamp", point_col: str = "point",
                 value_col: str = "value", unit_col: str | None = "unit", profile=None):
        self.path = path
        self.ts_col, self.point_col, self.value_col, self.unit_col = ts_col, point_col, value_col, unit_col
    ...
    def _pivot(self):
        ...
        idx = parse_timestamps(df[self.ts_col],
                               formats=[p.ts_format] if p.ts_format else None, dayfirst=p.dayfirst)
        vals = coerce_numeric(df[self.value_col], thousands=p.thousands, decimal=p.decimal)
        frame = pd.DataFrame({"point": df[self.point_col].astype(str).values, "value": vals.values},
                             index=idx)
        ...
        if self.unit_col and self.unit_col in df.columns:
            ...
            units = {r[self.point_col]: r[self.unit_col]

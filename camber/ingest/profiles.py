...
There is no public archive of raw per-vendor trend exports, so instead of shipping example files
we encode the **documented format conventions** of each vendor's export tool as a small
dataclass, and let the CSV loaders consume it. A profile sets the delimiter, encoding, header rows
to skip, the timestamp format + day-first flag, and the decimal/thousands separators; the loaders
fall back to the shared auto-detecting parsers (`camber.tsparse`, `camber.coerce`) when a field is
left ``None``, so a profile is a *hint*, never a hard requirement.
...
#: named presets keyed by lowercase vendor/tool id (documented conventions; override per site as
#: needed)
PROFILES: dict = {

...
def demand_response(load_kw: pd.Series, baseline_kw, *, event_start, event_end,
                    rebound_hours: float = 2.0) -> DemandResponseResult:
    ...
    if np.isscalar(baseline_kw):
        ...
        if arr.shape != (len(load_kw),):
            raise ValueError("baseline_kw array must have one value per load_kw sample; "
    ...
    rebound = float(((s[post] - base[post]).clip(lower=0) * hours[post]).sum()) if post.any() else 0.0

    return DemandResponseResult(
        event_hours=round(ev_hours, 2), baseline_kwh=round(base_kwh, 2),
        actual_kwh=round(actual_kwh, 2), energy_shed_kwh=round(energy_shed, 2),
...
def flexibility(load_kw: pd.Series, *, baseload_pct: float = 10.0) -> FlexibilityResult:
    ...
    return FlexibilityResult(
        baseload_kw=round(base, 2), peak_kw=round(peak, 2), mean_kw=round(mean, 2),
...
def carbon_aware_shift(load_kw: pd.Series, emissions_factor, shift_kwh: float) -> dict:
    ...
    """
    ef = (emissions_factor.reindex(load_kw.index) if isinstance(emissions_factor, pd.Series)
    ...
    return {"co2_saved_kg": round(shift_kwh * (hi - lo), 3),
            "ef_high": round(hi, 4), "ef_low": round(lo, 4),
...
class OperationScore:
    ...
    load_weighted_avg: float     # Σ(load·signal)/Σload — what the building actually paid/emitted per kWh
...
def operation_score(load_kw: pd.Series, signal, *, label: str = "price") -> OperationScore:
    ...
    """
    sig = (signal.reindex(load_kw.index) if isinstance(signal, pd.Series)
    ...
    if df.empty or df["l"].sum() <= 0:
        return OperationScore(label, float("nan"), float("nan"), float("nan"), float("nan"),
                              float("nan"), float("nan"))
    ...
    return OperationScore(
        signal=label, load_weighted_avg=round(lwa, 5), flat_avg=round(flat, 5),
        best_case=round(best, 5), worst_case=round(worst, 5),

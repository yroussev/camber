...
class FleetReport:
    ...
    def worst_by_cost(self):
        ...
        return sorted(
            [b for b in self.buildings if b.annual_cost_usd is not None],
            key=lambda b: -b.annual_cost_usd,
        )

    def by_efficiency(self):
        """Buildings ordered most- to least-efficient (lowest EUI first)."""
        return sorted([b for b in self.buildings if b.eui is not None], key=lambda b: b.eui)
    ...
    def to_html(self) -> str:
        ...
        for b in self.by_efficiency():
            ...
            parts.append(
                f"<tr><td>{e(b.site)}</td><td>{b.eui:.1f}</td><td>{pc}</td><td>{vm}</td></tr>"
            )
        parts.append(
            "</table><h2>Fault rollup</h2><table border='1' cellpadding='4'>"
            "<tr><th>Site</th><th>Faults</th><th>Warnings</th>"
            "<th>Top findings</th></tr>"
        )
        for b in self.worst_by_faults():
            parts.append(
                f"<tr><td>{e(b.site)}</td><td>{b.n_fault}</td>"
        ...
        if ranked:
            parts.append(
                "<h2>Recoverable waste</h2><table border='1' cellpadding='4'>"
                "<tr><th>Site</th><th>Estimated $/yr</th></tr>"
            )
            for b in ranked:
                parts.append(f"<tr><td>{e(b.site)}</td><td>${b.annual_cost_usd:,.0f}</td></tr>")
            parts.append("</table>")
        if self.fleet_top_rules:
            parts.append(
                "<h2>Most common findings fleet-wide</h2><ul>"
                + "".join(f"<li>{e(r)} ({n} buildings)</li>" for r, n in self.fleet_top_rules)
                + "</ul>"
            )
        return "\n".join(parts)
...
def build_fleet_report(
    ...
    for b in buildings:
        site = b.get("site", "")
        eui = b.get("eui")
        acts = [f for f in b.get("findings", []) if _attr(f, "severity", "info") in _ACTIONABLE]
    ...
    total = (
        round(sum(b.annual_cost_usd for b in summaries if b.annual_cost_usd is not None), 2)
        if any_cost
        else None
    )
    return FleetReport(

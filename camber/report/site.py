...
from .dashboard import (
    _SECTION_TITLES, _STYLE, _findings_table, _rules_map, _section_image, render_evidence_blocks,
)

_SC_STYLE = (".sc{font-size:15px;margin:10px 0}.grade{font-size:30px;font-weight:700}"
...
def _scorecard_html(sc) -> str:
    ...
    for c in sc.categories:
        rows.append(f"<tr><td>{_html.escape(c.category)}</td><td>{c.score:.0f}</td>"
                    f"<td>{c.grade}</td><td>{c.n_faults}</td><td>{c.n_warnings}</td></tr>")
    return (f"<p class='sc'><span class='grade {sc.overall_grade}'>{sc.overall_grade}</span> "
...
def build_site_report(df, *, findings=None, rules=None, frames=None, loads=None, price=None,
                      title: str = "CAMBER site report", sections=("A", "E", "I"),
                      rank_by: str = "cost", top_n: int = 20, normalize: bool = True) -> str:
    ...
    action plan (``loads``/``price`` cost the plan), and — with ``rules`` (and optional per-equipment
    ``frames``) — the pattern-J evidence. ``sections`` chooses which dashboard chart sections to
    include (A/B/E/I).
    """
    ...
    for letter in sections:
        img = _section_image(letter, df, spans=None, carpet_col=None, multitrend_cols=None,
                             normalize=normalize)

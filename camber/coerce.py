...
Vendor exports embed values in ways bare ``pd.to_numeric`` mangles: thousands separators (``1,234``),
European decimal commas (``1,5``), and a zoo of null/quality tokens (``N/A``, ``---``, ``Bad``,
``Comm Fail``). Status and command points carry *text* (``Running``/``Off``, ``Open``/``Closed``,
``Fault``/``Normal``). This module centralizes both so every adapter shares one coercion path instead of
a copy-pasted ``to_numeric(errors="coerce")``.
...
NULL_TOKENS = frozenset({
    "", "nan", "null", "none", "n/a", "na", "#n/a", "---", "--", "[-]", "-", "?",
    "bad", "no data", "nodata", "comm fail", "commfail", "err", "error", "#value!",
})

#: text that maps to 1.0 (ON / active / open) — extends the classic BAS on/off vocabulary
STATUS_ON = frozenset({
    "running", "on", "start", "started", "enabled", "active", "occupied", "true", "yes", "1",
    "open", "opened", "fault", "alarm", "tripped", "trip", "override", "hand", "manual",
})
#: text that maps to 0.0 (OFF / inactive / closed / normal)
STATUS_OFF = frozenset({
    "off", "stop", "stopped", "disabled", "inactive", "unoccupied", "false", "no", "0",
    "standby", "idle", "closed", "close", "normal", "auto", "ready", "reset", "ok",
})


def coerce_numeric(series, *, thousands: str | None = ",", decimal: str = ".",
                   null_tokens=NULL_TOKENS) -> pd.Series:

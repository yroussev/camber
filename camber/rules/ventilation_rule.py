...
class DemandControlledVentilation:
    ...
    def __init__(self, *, min_corr: float = 0.3, min_modulation: float = 0.1,
                 co2_setpoint: float | None = None, occupied_only: bool = True):
    ...
    def analyze(self, equip: str, frame: pd.DataFrame) -> Finding:
        oa_role = (Role.OA_AIRFLOW if Role.OA_AIRFLOW in frame.columns
                   else Role.OA_DAMPER if Role.OA_DAMPER in frame.columns else None)
        demand_role = Role.CO2 if Role.CO2 in frame.columns else (
            Role.OCCUPANCY if Role.OCCUPANCY in frame.columns else None)
        ...
        res = assess_dcv(frame[oa_role], frame[demand_role], occupied_mask=mask,
                         min_corr=self.min_corr, min_modulation=self.min_modulation,
                         co2_setpoint=setpoint, equip=equip)
        ...
        msg = {"static": "OA does not modulate with demand (DCV not functioning / fixed OA)",
               "uncorrelated": "OA modulates but not with demand",
               "functioning": "OA tracks demand (DCV functioning)"}[res.status]
        return Finding(rule=self.name, equip=equip, severity=severity,
                       metrics={"status": res.status, "correlation": res.correlation,
...
class VentilationRateProcedure:
    ...
    def __init__(self, *, area_sqft: float, population: float, space_type: str | None = None,
                 rp: float | None = None, ra: float | None = None, ez: float = 1.0,
                 occupied_only: bool = True, aggregate: str = "median",
                 under_tol: float = 0.9, over_factor: float = 1.5):
    ...
    def analyze(self, equip: str, frame: pd.DataFrame) -> Finding:
        mask = occupied_mask(frame.index) if self.occupied_only else None
        res = assess_62_1(frame[Role.OA_AIRFLOW], area_sqft=self.area_sqft,
                          population=self.population, space_type=self.space_type,
                          rp=self.rp, ra=self.ra, ez=self.ez, occupied_mask=mask,
                          aggregate=self.aggregate, under_tol=self.under_tol,
                          over_factor=self.over_factor, equip=equip)
        if res.status == "unknown":
            return Finding(rule=self.name, equip=equip, severity="info",
                           summary=f"{equip}: insufficient OA-flow data")
        severity = {"under": "fault", "over": "warn", "adequate": "ok"}[res.status]
        if res.status == "under":
            tail = (f"under-ventilated: {res.measured_cfm:.0f} cfm OA vs {res.required_cfm:.0f} "
                    f"required (62.1 VRP), deficit {res.deficit_cfm:.0f} cfm")
        elif res.status == "over":
            tail = (f"over-ventilated: {res.measured_cfm:.0f} cfm vs {res.required_cfm:.0f} "
        ...
        return Finding(rule=self.name, equip=equip, severity=severity,
                       metrics={"required_cfm": res.required_cfm, "measured_cfm": res.measured_cfm,
                                "ratio": res.ratio, "deficit_cfm": res.deficit_cfm,
                                "status": res.status, "rp": res.rp, "ra": res.ra, "ez": res.ez},

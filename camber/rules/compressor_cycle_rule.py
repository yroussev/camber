...
hysteresis. Reuses the generic on/off start counter (:func:`camber.boilercycle.analyze_boiler_cycling`)
over the compressor status trend. ``max_starts_per_day`` is manufacturer / min-off-time dependent, so
...
class CompressorShortCycle:
    ...
    def analyze(self, equip: str, frame: pd.DataFrame) -> Finding:
        ...
        return Finding(
            ...
            metrics={"starts_per_day": res.starts_per_day,
                     "max_starts_per_day": self.max_starts_per_day,
                     "runtime_pct": res.runtime_pct, "n_starts": res.n_starts,
                     "n_days": res.n_days},
            summary=(f"{equip}: {res.starts_per_day:.1f} compressor starts/day "

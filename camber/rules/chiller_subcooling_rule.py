...
# Plausibility bounds for subcooling, degF -- shared with camber.sensorhealth. **Not** the 0-50
# degF approach default: flash gas in the liquid line (severe undercharge) reads ~0 and, with
# transducer error, a few degF below 0 -- that *is* the fault, so it must be scoreable. Only
# sentinel codes and dead-channel arithmetic fall outside (-20, 80).
SUBCOOLING_PLAUSIBLE_F = PHYSICAL_BOUNDS[Role.SUBCOOLING_TEMP]
    ...
    **Alarm symmetry.** Scoring is symmetric in magnitude and signed in reporting: :meth:`_severity`
    ...
    def _frozen_baseline(self, equip, base_frame, caveats, gate=None):
        ...
        return _chillerfit.freeze_baseline(
            ...
            metric_range=SUBCOOLING_PLAUSIBLE_F,
    ...
    def analyze_periods(self, equip: str, baseline: pd.DataFrame, current: pd.DataFrame) -> Finding:
        ...
        metrics["subcooling_min_tons"] = gate[0]
        _chillerfit.fit_notes(frozen, caveats, metrics, "subcooling")
        ...
        try:
            ...
            run = monitor.run(
                ...
                tons_col="tons",
                min_tons=gate[0],
                approach_range=SUBCOOLING_PLAUSIBLE_F,

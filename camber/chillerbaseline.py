"""Load-normalized chiller approach baseline: fit ``approach ~ f(tons)`` and score drift against it.
...
This module supplies the missing piece: a **fitted baseline** of approach against load, retaining
...
The fit is ordinary least squares of degree 1: approach-vs-load is close to linear across a
chiller's operating band, and two parameters stay stable on the few hundred to few thousand hourly
...
:meth:`ApproachBaseline.predict` deliberately duck-types the ``predict`` callable that
...
class ApproachBaseline:
    ...
    def predict(self, tons):
        """Expected approach (degF) at ``tons``; scalar in -> float out, array in -> array out.
    ...
    def residual(self, tons, approach_f):
        """Actual minus expected approach (degF). **Positive means wider than baseline.**"""
        actual = np.asarray(approach_f, dtype=float)
        out = actual - np.asarray(self.predict(tons), dtype=float)
        return float(out) if out.ndim == 0 else out

    def z(self, tons, approach_f):
        """:meth:`residual` in baseline residual sigmas (NaN if the fit had no scatter)."""
        resid = np.asarray(self.residual(tons, approach_f), dtype=float)
    ...
    @classmethod
    def from_dict(cls, d: dict) -> ApproachBaseline:
...
@dataclass
class ApproachDrift:
    """How far a period's approach sits above its fitted baseline, at matched load."""

    n_current: int  # samples scored after guards
    drift_f: float  # median residual vs the baseline fit, degF (+ = wider = degraded)
...
def _clean(
    ...
    *,
    ...
    """Rows usable for fitting or scoring: both points present, real load, physical approach.

    Below ``min_tons`` the chiller is effectively unloaded and the approach carries no information
    about heat-exchanger condition; the approach bounds drop sensor dropouts and impossible values.
    Returns a two-column frame named ``tons``/``approach``, preserving the original index.
    """
    if approach_col not in frame.columns or tons_col not in frame.columns:
        return pd.DataFrame(columns=["tons", "approach"])
    w = pd.DataFrame(
        {
            "tons": pd.to_numeric(frame[tons_col], errors="coerce"),
            "approach": pd.to_numeric(frame[approach_col], errors="coerce"),
        },
        index=frame.index,
    ).dropna()
    lo, hi = approach_range
    return w[(w["tons"] >= min_tons) & w["approach"].between(lo, hi)]
...
def fit_approach_baseline(
    ...
    *,
    ...
    tons_col: str = "tons",
    min_tons: float = 5.0,  # below this the ratio carries no fouling information
    approach_range: tuple[float, float] = (0.0, 50.0),  # physical-ish degF bounds
    min_samples: int = 30,  # too few points and the slope isn't identified
    min_tons_span: float = 10.0,  # too narrow a load range and the slope isn't identified
) -> ApproachBaseline | None:
    """Fit ``approach ~ intercept + slope * tons`` over a baseline period; retain residual sigma.

    ``frame`` is indexed by time and carries an approach column and a load column (derive the latter
    ...
    y = w["approach"].to_numpy(dtype=float)
    if float(x.max() - x.min()) < min_tons_span:
        return None
...
def drift_stats(
    ...
    *,
    ...
    ``drift_f`` is the **median** residual (actual minus baseline-predicted approach at the same
    ...
    resid = np.asarray(baseline.residual(tons, w["approach"].to_numpy(dtype=float)), dtype=float)

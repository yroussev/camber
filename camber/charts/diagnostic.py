...
def diagnostic_scatter(
    ...
    *,
    ...
    if len(xv):
        order = np.argsort(xv)
        ax.fill_between(
            xv[order], lo[order], hi[order], color="#8fd19e", alpha=0.25, label="expected", zorder=1
        )
    ax.scatter(
        xv[~violating],
        yv[~violating],
...
def economizer_template(
    *, high_limit_f: float = 65.0, min_damper: float = 0.2, open_min: float = 0.5
) -> DiagnosticTemplate:
...
TEMPLATES: dict[str, DiagnosticTemplate] = {
    "sat_reset": reset_line(
        ...
        tol=2.0,
        name="sat_reset",
        cite="ASHRAE G36 SAT reset",
        xlabel="OAT (°F)",
        ylabel="SAT (°F)",
    ),
    "chw_reset": reset_line(
        Role.OAT,

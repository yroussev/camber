...
def box_reheat_figure(
    df, equip, *, oat=None, valve_thr=5.0, cooling_cutoff_f=65.0, occupied_only=True
):
    ...
    if "HWValve" in work.columns:
        ax1.plot(work.index, work["HWValve"], color="#cc3333", lw=0.6, label="HW reheat valve %")
    ax1.set_ylabel("Reheat valve (%)", color="#cc3333")
    ax1.set_ylim(0, 100)
    if "ActFlow" in work.columns:
        axf = ax1.twinx()
        axf.plot(work.index, work["ActFlow"], color="#888888", lw=0.5, alpha=0.7, label="Airflow")
    ...
    if "SpaceTemp" in work.columns:
        ax2.plot(work.index, work["SpaceTemp"], color="#2a7", lw=0.7, label="Space temp")
    ...
    if oat is not None:
        ...
        if "HWValve" in work.columns:
            ...
            ax2.scatter(
                work.index[hot],
                work["SpaceTemp"][hot]
                if "SpaceTemp" in work
                else [cooling_cutoff_f] * int(hot.sum()),
                color="red",
                s=4,
                zorder=5,
                label="reheat @ OAT>65°F",
            )

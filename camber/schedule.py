...
def detect_schedule(
    ...
    *,
    ...
    using the 10th/90th percentiles (robust to spikes). An hour-of-week slot is "on" when it is
    above threshold in at least ``min_fraction`` of the weeks observed.
    """
    s = series.dropna()
    if s.empty:
...
def compare_schedule(detected: WeeklySchedule, stated_on_slots) -> dict:
    ...
    agree = sum(
        1
        for slot in ((d, h) for d in range(7) for h in range(24))
        if (slot in det) == (slot in stated)
    )
    return {
        "extra_runtime_slots": extra,

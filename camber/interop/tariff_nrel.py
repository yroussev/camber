...
def _require_pysam():
    try:
        ...
        raise ImportError(
            'the NREL PySAM bridge needs the optional extra: pip install "camber-toolkit[tariff]"'
        ) from e
...
def bill_with_pysam(urdb_rate: dict, load_kw_8760, *, analysis_period: int = 1) -> dict:
    ...
    annual = float(
        out.get("utility_bill_w_sys_year1") or out.get("elec_cost_with_system_year1") or 0.0
    )

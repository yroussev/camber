...
def _require():
    try:
        ...
        raise ImportError(
            'the pvlib bridge needs the optional extra: pip install "camber-toolkit[pv]"'
        ) from e
    return pvlib
...
def pvwatts_expected_kwh(
    poa_kwh_m2: float,
    rated_kw: float,
    *,
    cell_temp_c: float = 25.0,
    gamma_pdc: float = -0.0047,
    ...
    dc = pvlib.pvsystem.pvwatts_dc(
        1000.0 * float(poa_kwh_m2), float(cell_temp_c), float(rated_kw), gamma_pdc
    )
